<template>
	<div>
		...
		<el-dialog @open="open" :title="dialogTitle" @before-close="handleOk" width="70%" v-model="state.dialogVisible" append-to-body>
			<div class="table-demo-container layout-padding">
				<div class="table-demo-padding layout-padding-view layout-padding-auto">
					<vz-table ref="vzTableRef" :tableData="state.tableData" @row-click="rowclick" @row-dblclick="doubleClick"></vz-table>
				</div>
			</div>
			<template #footer>
...
const state = reactive({
	...
	tableData: {
		...
		// 表头内容（必传，注意格式）
		header: [
			{ key: 'versionName', colWidth: '150', title: '版本名称', type: 'text', isCheck: true },
			{
				key: 'versionType',
				colWidth: '100',
				title: '版本类型',
			...
			{ key: 'effectiveDays', colWidth: '100', title: '有效天数', type: 'text', isCheck: true },
			{ key: 'remarks', colWidth: '350', title: '备注', type: 'text', align: 'left', isCheck: true },
			{ key: 'enabled', colWidth: '90', title: '是否启用', type: 'boolean', isCheck: true },
			{ key: 'creator', colWidth: '100', title: '创建人', type: 'user', isCheck: true },
			{ key: 'createStamp', colWidth: '160', title: '创建时间', type: 'datetime', isCheck: true },
		],
		// 配置项（必传）
		config: {
			listApi: useVersionApi().findAll, //列表接口
			delApi: useVersionApi().delete, //删除接口
			total: 0, // 列表总数
...
//点击确定
const handleOk = () => {
	if (Object.keys(companyList.value).length === 0 && select.value.length === 0) {
...
const getAgreementName = async (id: number | string) => {
	if (!id) return;
	const { data } = await useVersionApi().findById(id);
