<template>
  <div class="menu-search-dialog">
    <div class="layout-navbars-icon">
      <i :class="'iconfont icon-sousuo'" class="toolBar-icon" @click="handleOpen" title="菜单搜索"></i>
    </div>
    <el-dialog v-model="isShowSearch" destroy-on-close :modal="false" :show-close="false" fullscreen @click="closeSearch">
      ...
      >
        ...
        <template #default="{ item }">
          <svg-icon class="breadcrumb-icon" :name="item.meta.icon" v-if="item.meta.icon"></svg-icon>
          <!-- <el-icon>
