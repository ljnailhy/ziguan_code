...
<script setup lang="tsx" name="paymentCollectionTarget">
...
import {
  PaymentCollectionTargetPageRequest,
  PaymentCollectionTargetDTO
} from "@/api/modules/compensatory/paymentCollectionTarget/interface"; // 当前页面的数据类型 DTO是列表返回值的数据类型
...
// 表格配置项
const columns = reactive<ColumnProps<PaymentCollectionTargetDTO>[]>([
  ...
  { type: "index", label: "序号", width: 60 },
  {
    ...
    search: {
      el: "date-picker",
      span: 1,
      props: { type: "year", format: "YYYY", valueFormat: "x" }
    }
  },
  {
    prop: "targetMoney",
...
const getTableList = (params: PaymentCollectionTargetPageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return paymentCollectionTarget().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: PaymentCollectionTargetDTO) => {
  await useHandleData(paymentCollectionTarget().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<PaymentCollectionTargetDTO> = {}) => {
  const params = {
    dialogName: "paymentCollectionTarget_createUpdate",
    title,
    showBtn: false,
    id: row?.id,
    isView: title === "查看",
    api: title === "新增" ? paymentCollectionTarget().add : title === "编辑" ? paymentCollectionTarget().update : undefined,
