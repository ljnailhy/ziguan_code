  ...
  <div class="table-box">
    ...
    >
      <template #tableHeader>
        <el-button type="primary" v-auth="'allocation'" @click="openAllocation('编辑')">{{
          route.name == "unallocationInfo" ? "分配" : "重新分配"
        }}</el-button>
      ...
      <template #documentType="scope">
        <div class="flx">
          ...
          <dict-enum
            ...
            :value="scope.row.subjectInfoRequest && scope.row.subjectInfoRequest.documentType"
          ></dict-enum>
        </div>
        <div class="flx">
          证件号码：
          {{ scope.row.subjectInfoRequest && scope.row.subjectInfoRequest.documentNumber }}
        </div>
      </template>
...
const ayncData = () => {
  ...
  return projectInfo()
    .syncCompensatory(params)
    .then((res: EmptyObjectType) => {
      if (res.code == 0 && res.errorMessage && res.errorMessage.includes("证件号")) {
        ElMessageBox.confirm(res.errorMessage, "温馨提示", {
          distinguishCancelAndClose: true,
          confirmButtonText: "全部同步",
          cancelButtonText: "部分同步"
        })
          .then(() => {
            //点击全部同步
            params.isSync = true;
...
const columns = reactive<ColumnProps<ProjectInfoDTO>[]>([
  {
    ...
    fixed: "left",
    width: 70,
    isShow: !props.isShowBtn || route.name == "unallocationInfo" || route.name == "alreadyallocationInfo"
  },
...
const getTableList = async (params: ProjectInfoPageRequest) => {
  ...
  const state = (projectState.value = projectStateList
    ...
    .map(item => item.id));
  const PROJECT_STATE_01 = projectStateList
    .filter(item => item.itemCode == "PROJECT_STATE_01") // 筛选条件
    .map(item => item.id);
  ...
  if (route.name == "alreadyallocationInfo") {
    const merged = Object.assign({}, newParams, {
      projectStateList: state
    });
    return projectInfo().findAll(merged);
  }
  if (route.name == "unallocationInfo") {
    const merged = Object.assign({}, newParams, {
      projectStateList: PROJECT_STATE_01
    });
    return projectInfo().findAll(merged);
  }
...
const openAllocation = (title: string, row: any = {}) => {
  if (vzTableRef.value?.selectedList.length == 0) {
    return ElMessage.warning("请先选择项目");
  }
  const params = {
    dialogName: "allocationInfo_createUpdate",
    title,
    showBtn: true,
    id: row?.id,
    objList: vzTableRef.value?.selectedList,
    isView: title === "查看",
    api: title === "新增" ? allocationInfo().add : title === "编辑" ? allocationInfo().update : undefined,
    getTableList: vzTableRef.value?.getTableList
  };
...
const openDrawer = (title: string, row: any = {}) => {
  if (title === "查看") {
    // /proceeding/projectInfo/detail/
    console.log(route.name);
    const path =
      route.name === "proceedingProjectInfo" ? "/compensatory/projectInfo/detail/" : "/compensatory/projectInfo/detail/";
    router.replace({
      path: `${path}${row.id}`,
      query: {
        product: row.product
