<template>
  <div class="table-box">
    <vz-table ref="vzTableRef" :columns="columns" :request-api="getTableList" :init-param="initParam">
      ...
      <template #allocationIllustrate="scope">
        <div v-html="scope.row.allocationIllustrate"></div>
      </template>
      <!-- 表格操作 -->
...
<script setup lang="tsx" name="allocationInfo">
...
import { AllocationInfoPageRequest, AllocationInfoDTO } from "@/api/modules/compensatory/allocationInfo/interface"; // 当前页面的数据类型 DTO是列表返回值的数据类型
...
// 表格配置项
const columns = reactive<ColumnProps<AllocationInfoDTO>[]>([
  { type: "selection", fixed: "left", width: 70, isShow: false },
...
const getTableList = (params: AllocationInfoPageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return allocationInfo().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: AllocationInfoDTO) => {
  await useHandleData(allocationInfo().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<AllocationInfoDTO> = {}) => {
  const params = {
