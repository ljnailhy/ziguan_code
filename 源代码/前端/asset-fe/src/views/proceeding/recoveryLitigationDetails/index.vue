...
<script setup lang="tsx" name="recoveryLitigationDetails">
...
interface recoveryLitigationDetailsProps {
  filterData?: { [key: string]: any };
  selectedId?: number;
}
const props = withDefaults(defineProps<recoveryLitigationDetailsProps>(), {
  filterData: () => ({}),
  selectedId: () => 0
});
const getIllustrateLabel = () => {
  if (props.filterData.litigationType == LitigationTypeEnum.FINAL) {
    return "终本";
  }
  if (props.filterData.litigationType == LitigationTypeEnum.OTHER) {
    return "其他";
  }
  if (props.filterData.litigationType == LitigationTypeEnum.DROP_LAWSUIT) {
    return "撤诉";
  }
  if (props.filterData.litigationType == LitigationTypeEnum.CLOSE_CASE) {
    return "结案";
  }
  if (props.filterData.litigationType == LitigationTypeEnum.PRESERVATION) {
    return "保全";
  }
};
// 表格配置项
const columns = reactive<ColumnProps<RecoveryLitigationDetailsDTO>[]>([
  ...
  //   label: "立案案号",
  //   minWidth: 120,
  //   isShow: props.filterData.litigationType && props.filterData.litigationType == LitigationTypeEnum.DROP_LAWSUIT
  // },
  {
    prop: "preservationCode",
    label: "保全案号",
    minWidth: 120,
    isShow: props.filterData.litigationType && props.filterData.litigationType == LitigationTypeEnum.PRESERVATION
  },
  {
    type: "date",
    prop: "detailsDate",
    label: getIllustrateLabel() + "时间",
    width: 120,
    search: {
...
// 删除用户信息
const deleteAccount = async (params: RecoveryLitigationDetailsDTO) => {
  openDrawer("取消", params);
  await useHandleData(recoveryLitigationDetails().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<RecoveryLitigationDetailsDTO> = {}) => {
