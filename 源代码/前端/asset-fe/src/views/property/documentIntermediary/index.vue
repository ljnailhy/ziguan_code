...
<script setup lang="tsx" name="documentIntermediary">
...
import { DocumentIntermediaryPageRequest, DocumentIntermediaryDTO } from "@/api/modules/property/documentIntermediary/interface"; // 当前页面的数据类型 DTO是列表返回值的数据类型
...
// 表格配置项
const columns = reactive<ColumnProps<DocumentIntermediaryDTO>[]>([
...
const getTableList = (params: DocumentIntermediaryPageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return documentIntermediary().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: DocumentIntermediaryDTO) => {
  await useHandleData(documentIntermediary().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<DocumentIntermediaryDTO> = {}) => {
  const params = {
    dialogName: "documentIntermediary_createUpdate",
    title,
    showBtn: false,
    id: row?.id,
    isView: title === "查看",
    api: title === "新增" ? documentIntermediary().add : title === "编辑" ? documentIntermediary().update : undefined,
