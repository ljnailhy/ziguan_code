...
<script setup lang="tsx" name="propertyBill">
...
import { PropertyBillPageRequest, PropertyBillDTO } from "@/api/modules/property/propertyBill/interface"; // 当前页面的数据类型 DTO是列表返回值的数据类型
...
// 表格配置项
const columns = reactive<ColumnProps<PropertyBillDTO>[]>([
...
const getTableList = (params: PropertyBillPageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return propertyBill().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: PropertyBillDTO) => {
  await useHandleData(propertyBill().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<PropertyBillDTO> = {}) => {
  const params = {
    dialogName: "propertyBill_createUpdate",
    title,
    showBtn: false,
    id: row?.id,
    isView: title === "查看",
    api: title === "新增" ? propertyBill().add : title === "编辑" ? propertyBill().update : undefined,
