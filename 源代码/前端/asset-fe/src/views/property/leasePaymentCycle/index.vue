...
<script setup lang="tsx" name="leasePaymentCycle">
...
import { LeasePaymentCyclePageRequest, LeasePaymentCycleDTO } from "@/api/modules/property/leasePaymentCycle/interface"; // 当前页面的数据类型 DTO是列表返回值的数据类型
...
// 表格配置项
const columns = reactive<ColumnProps<LeasePaymentCycleDTO>[]>([
...
const getTableList = (params: LeasePaymentCyclePageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return leasePaymentCycle().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: LeasePaymentCycleDTO) => {
  await useHandleData(leasePaymentCycle().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<LeasePaymentCycleDTO> = {}) => {
  const params = {
    dialogName: "leasePaymentCycle_createUpdate",
    title,
    showBtn: false,
    id: row?.id,
    isView: title === "查看",
    api: title === "新增" ? leasePaymentCycle().add : title === "编辑" ? leasePaymentCycle().update : undefined,
