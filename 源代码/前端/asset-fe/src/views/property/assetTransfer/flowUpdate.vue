  ...
  <div>
    <vz-card title="资产信息">
      <form-table
        ...
        :filter-data="{ propertyStateList: ['IDLE', 'SELF_USE', 'LEASED', 'PART_LEASE', 'PART_TRANSFER'] }"
        ...
        <el-row :gutter="35">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
            <el-form-item label="评估有效期截止时间" prop="estimateEndDate">
              <el-date-picker
                v-model="form!.estimateEndDate"
                type="date"
                placeholder="请选择评估有效期截止时间"
                class="w100"
                :disabled="drawerProps.isView"
              />
            </el-form-item>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
            <el-form-item label="联系人" prop="contacts">
              <el-input v-model="form!.contacts" placeholder="请输入联系人" clearable :disabled="drawerProps.isView"></el-input>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
    ...
    <vz-card title="中介信息">
      ...
      <!-- <el-dialog v-model="dialogVisible" title="登记客户线索" width="80%">
        <form-table
        ...
        <el-row :gutter="35">
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
            <el-form-item label="受让人" prop="customId">
              <vzCustomerInfo v-model:value="form!.customId" select-type="radio" :default-value="form!.customId"></vzCustomerInfo>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
            <el-form-item label="成交日期" prop="dealDate">
              <el-date-picker v-model="form!.dealDate" type="date" value-format="x" placeholder="请选择成交日期" class="w100" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" class="mb20">
            <el-form-item label="转让日期" prop="transferOwnershipDate">
              <el-date-picker
                @change="changeDealMoney"
                v-model="form!.transferOwnershipDate"
                type="date"
                placeholder="请选择转让日期"
                class="w100"
              />
    ...
    <vz-card title="转让产权信息">
      <form-table
        ...
        select-type="radio"
        :filter-data="{ doIds: assetInfo.data && assetInfo.data.map(item => item.propertyId) }"
        :readonly="drawerProps.readonly"
      ></form-table>
    </vz-card>
    <!-- <vz-card title="转让收入明细">
      <form-table
        ref="assetMoneyRef"
...
const bakpropertyRightInfoList = ref([]);
...
const changeDealMoney = () => {
  // 计算转让产权信息分录中的转让金额
  if (propertyRightInfoList.value.data.length) {
    const assetMoneyData = JSON.parse(JSON.stringify(propertyRightInfoList.value.data));
    const area = assetMoneyData.reduce((accumulator, currentValue) => {
    ...
    const price = form.value!.dealMoney / area;

    propertyRightInfoList.value.data = propertyRightInfoList.value.data.map(res => {
      return {
        id: res.id,
        parentId: res.parentId,
        propertyInfoId: res.propertyInfoId,
        propertyCode: res.propertyCode,
        assetUnitState: res.assetUnitState,
        originalValue: res.originalValue,
        area: res.area,
        propertyMoney: price * res.area,
        propertyOwner: res.propertyOwner,
        propertyEndDate: res.propertyEndDate,
        propertyTransferOwnership: res.propertyTransferOwnership,
        assetUse: res.assetUse,
        address: res.address,
        remark: res.remark
      };
    });
  ...
  //       propertyDate: form.value!.transferOwnershipDate
  //     };
  //   });
  // }
};
...
const selectCustomer = (val: any, index: number, item: any, row: any) => {
  if (item.prop === "Contacts") {
    ...
    customerArray.value.data[index].customPhone = val.contactsPhone;
...
const saveFun = async () => {
  ...
  // form.value.propertyRightInfoRequestList = form.value.propertyRightInfoRequests;
...
const handleSave = async () => {
  ...
  //   ElMessageBox.confirm("请选择是否挂网?", "温馨提示", {
  ...
  //   })
  //     .then(async () => {
  //       form.value.isAgainAuction = true;
  //       try {
  //         await assetTransfer()?.update(form.value);
  //         ElMessage.success({ message: "保存资产转让成功！" });
  //         drawerProps.value.getTableList?.();
  ...
  //       // 用户点击取消按钮或对话框关闭
  //       if (action === "cancel") {
  //         form.value.isAgainAuction = false;
  //         try {
  //           await assetTransfer()?.update(form.value);
  //           ElMessage.success({ message: "保存资产转让成功！" });
  //           drawerProps.value.getTableList?.();
