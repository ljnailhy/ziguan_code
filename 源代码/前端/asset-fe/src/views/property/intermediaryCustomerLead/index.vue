...
<script setup lang="tsx" name="intermediaryCustomerLead">
...
import {
  IntermediaryCustomerLeadPageRequest,
...
// 表格配置项
const columns = reactive<ColumnProps<IntermediaryCustomerLeadDTO>[]>([
...
const getTableList = (params: IntermediaryCustomerLeadPageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return intermediaryCustomerLead().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: IntermediaryCustomerLeadDTO) => {
  await useHandleData(intermediaryCustomerLead().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<IntermediaryCustomerLeadDTO> = {}) => {
  const params = {
    dialogName: "intermediaryCustomerLead_createUpdate",
    title,
    showBtn: false,
    id: row?.id,
    isView: title === "查看",
    api: title === "新增" ? intermediaryCustomerLead().add : title === "编辑" ? intermediaryCustomerLead().update : undefined,
