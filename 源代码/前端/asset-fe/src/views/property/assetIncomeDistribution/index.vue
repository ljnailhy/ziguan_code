...
<script setup lang="tsx" name="assetIncomeDistribution">
...
import {
  AssetIncomeDistributionPageRequest,
...
// 表格配置项
const columns = reactive<ColumnProps<AssetIncomeDistributionDTO>[]>([
...
const getTableList = (params: AssetIncomeDistributionPageRequest) => {
  let newParams = JSON.parse(JSON.stringify(params));
  delete newParams.createStamp;
  return assetIncomeDistribution().findAll(newParams);
};

// 删除用户信息
const deleteAccount = async (params: AssetIncomeDistributionDTO) => {
  await useHandleData(assetIncomeDistribution().delete, { id: params.id }, `此操作将永久删除该条数据，是否继续`);
  vzTableRef.value?.getTableList();
};

// 打开 dialog(新增、查看、编辑)
const dialogRef = ref<any>(null);
const openDrawer = (title: string, row: Partial<AssetIncomeDistributionDTO> = {}) => {
  const params = {
    dialogName: "assetIncomeDistribution_createUpdate",
    title,
    showBtn: false,
    id: row?.id,
    isView: title === "查看",
    api: title === "新增" ? assetIncomeDistribution().add : title === "编辑" ? assetIncomeDistribution().update : undefined,
