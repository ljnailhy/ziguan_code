...
// 引入组件
const tables = defineAsyncComponent(() => import("@/views/property/propertyInfo/index.vue"));
...
const props = withDefaults(defineProps<propertyInfoProps>(), {
  placeholder: "",
  disabled: false,
  defaultValue: "",
  companyId: "",
...
const getName = async (id: number | string) => {
  if (!id) return "";
  const data: any = await useBaseStore().findAssetName(id);
