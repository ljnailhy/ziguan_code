...
// 引入组件
const tables = defineAsyncComponent(() => import("@/views/proceeding/recoveryJudgement/assemblyIndex.vue"));
...
const getName = async (id: number | string) => {
  if (!id) return "";
  const data: any = await recoveryJudgement().findById(id);
  return data.data.fillingCode;
};
