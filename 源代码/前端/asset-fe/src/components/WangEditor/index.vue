...
import { nextTick, computed, inject, shallowRef, onBeforeUnmount, watch, ref } from "vue";
...
const imgurl = ref("http://222.240.31.178:10000/minio");
if (import.meta.env.DEV || location.origin == "http://192.168.1.100") {
  imgurl.value = "http://192.168.1.100/minio";
}
// 实列化编辑器
...
props.editorConfig.MENU_CONF!["uploadImage"] = {
  async customUpload(file: File, insertFn: InsertFnTypeImg) {
    ...
    try {
      ...
      insertFn(imgurl.value + data[0].fileUrl);
      debugger;
    } catch (error) {
      console.log(error);
    }
