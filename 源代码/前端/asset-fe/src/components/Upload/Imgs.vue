  ...
  <div class="upload-box">
    ...
    >
      ...
      <template #file="{ file, index }">
        <img v-if="_fileList && _fileList.length > 0" :src="'/minio' + _fileList[index]?.fileUrl" class="upload-image" />
        <img v-else :src="'/minio' + file.url" class="upload-image" />
...
const uploadSuccess = (response: { fileUrl: string } | undefined, uploadFile: any) => {
  if (!response) return;
  uploadFile.fileUrl = response[0].fileUrl;
  uploadFile.fileName = uploadFile.name;
  uploadFile.annexName = uploadFile.name;
  uploadFile.fileSize = uploadFile.size > 0 ? uploadFile.size : 0;
...
const handleRemove = (file: any) => {
  _fileList.value = _fileList.value.filter((item: any) => item.fileUrl !== file.fileUrl || item.fileName !== file.fileName);
  emit("update:fileList", _fileList.value);
};
...
const handlePictureCardPreview: any["onPreview"] = file => {
  viewImageUrl.value = file.fileUrl!;
