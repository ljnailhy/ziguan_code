...
export function parseMetaRecursively(item: any) {
  if (item.meta && typeof item.meta === "string") {
    item.meta = JSON.parse(item.meta);
  }
  if (item.menuPath && !item.path) {
...
/**
 * @description
 * @param {Array} arr1 备份的数据
 * @param {Array} arr2 分录的数据
 * */
export function deduplicateArraysById(arr1, arr2) {
  if (!arr1) return arr2;
  ...
  return [...arr2, ...filteredArr1];
}

export const mergerdArrayFun = (arr: any, arr1: any) => {
  if (!arr) {
...
//手机号码校验
export const textPhone = (rule, value, callback) => {
  const mobile = value && value.replace(/\s/g, "");
  if (!mobile) return callback(new Error("请填写正确的手机号码"));

  let tel = /^0[0-9]{2,3}[0-9]{7,8}$/;
  let phone = /^(((13[0-9]{1})|(15[0-9]{1})|(14[0-9]{1})|(18[0-9]{1})|(16[0-9]{1})|(17[0-9]{1})|(19[0-9]{1}))+\d{8})$/;
  // 检查是否以非零数字开头
  const isNonZeroStart = /^[1-9]/.test(mobile);
  const containsSpecialChars = /[^a-zA-Z0-9]/.test(mobile);
  if (mobile.length == 11 && isNonZeroStart && !containsSpecialChars) {
    //手机号码
    ...
    const mobiles = value.replace(/\D/g, "").toString();
