...
public interface WriteOffService {
  ...
  @PostMapping(CONTEXT)
  Result<WriteOffDTO> add(@RequestBody WriteOffRequest request);
  ...
  Result<WriteOffDTO> submit(@RequestBody WriteOffRequest request);
  ...
  @PutMapping(CONTEXT)
  Result<WriteOffDTO> update(@RequestBody WriteOffRequest request);
  ...
  Result<WriteOffDTO> findById(@ApiParam("主键") @PathVariable Long id);
  ...
  Result<List<WriteOffDTO>> findAll(@ApiParam("当前页码") @RequestParam(defaultValue = "1") Integer current,
      @ApiParam("每页大小") @RequestParam(defaultValue = "50") Integer size,
      @RequestBody WriteOffPageRequest request);
