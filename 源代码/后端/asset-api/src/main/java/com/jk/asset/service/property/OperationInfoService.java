...
public interface OperationInfoService {
  ...
  String CONTEXT = "/operation/info";

  /**
   * 新增
   *
   * @param request 运营信息表入参
  ...
  @PostMapping(CONTEXT)
  Result<OperationInfoDTO> add(@RequestBody OperationInfoRequest request);
  ...
  @PutMapping(CONTEXT)
  Result<OperationInfoDTO> update(@RequestBody OperationInfoRequest request);
  ...
  Result<OperationInfoDTO> findById(@ApiParam("主键") @PathVariable Long id);

  /**
   * 提交
   * @param request 运营信息表入参
   * @return com.jk.common.model.Result<com.jk.asset.model.dto.property.LeaseInfoDTO>
   * @author wangshuai
   * @since 2024/7/13 17:51
   **/
  @ApiOperation(value = "提交-submit", notes = "详细描述")
  @PutMapping(CONTEXT+"/submit")
  Result<OperationInfoDTO> submit(@RequestBody OperationInfoRequest request);

  @ApiOperation(value = "反写相关信息-writeBackProject", notes = "详细描述")
  @GetMapping(CONTEXT + "/writeBack/{id}")
  Result<OperationInfoDTO> writeBackProject(@ApiParam("主键") @PathVariable Long id);
  ...
  Result<List<OperationInfoDTO>> findAll(@ApiParam("当前页码") @RequestParam(defaultValue = "1") Integer current,
      @ApiParam("每页大小") @RequestParam(defaultValue = "50") Integer size,
      @RequestBody OperationInfoPageRequest request);
